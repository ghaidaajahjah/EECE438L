...
class TeamFormTest(LiveServerTestCase):
  ...
  def testform(self):
    ...
    team_year= selenium.find_element('id','id_year_founded')

    submit = selenium.find_element('id','submit_button')

    #populate the form with data
    team_id.send_keys('123456')
    team_name.send_keys('some team')
    team_city.send_keys('beirut')
    team_year.send_keys('2012')

    #submit form
    submit.send_keys(Keys.RETURN)

    #check result; page source looks at entire html document
    assert '123456' in selenium.page_source